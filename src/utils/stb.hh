...
namespace stb
{
                    ...
                    for (auto entry : skips)
                    {
                        if (entry == i && entry < end)
                        {
                            ...
                            result[conversions++] = str()[idx] == mask ?
                                                      masked :
                                                      (one_char ? detail::char_to_hex(str()[idx]) : detail::concat_hex(detail::char_to_hex(str()[idx]), detail::char_to_hex(str()[idx + 1])));
                        }
                    }
                }
    ...
    using to_bytes = basic_compiletime_string_to_byte_array_data<' ', '?', -2>;
} // namespace stb

#define STB(x) stb::to_bytes::getter<x>::value
